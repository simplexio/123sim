...
/**
 * This class builds an OHAP path to be used when sending SET, GET and LISTEN
 * and UNLISTEN commands to the OHAP central unit.
 * <p>
 * 
 * The path follows this structure:
 * 
 * <pre>
 * LISTEN /container:room-1/actuator:light-1
 * </pre>
 * 
 * when using GET, LISTEN and UNLISTEN. The path contains a device id with type
 * information.
 * <p>
 * When setting values of a device, with the SET command, the path also includes
 * the new value for the device. For example:
 * <p>
 * 
 * <pre>
 * SET /container:room-1/actuator:light-1/state/value true
 * </pre>
 * 
...
public class OHAPPathBuilder implements Visitor {
	...
	/**
	 * Call this method to get the path for a device.
	 * <p>
	 * If the device is a concrete device and the withValue parameter indicates
	 * that value should be included in the path, it is put there. Otherwise,
	 * only the device hierarchy is included in the path.
	 * 
	 * @param device
	 *            The device to get path for.
	 * @param withValue
	 *            Do we want the value of the device in the path.
	...
	/**
	 * Constructs a path in to the buffer, using the device's id and type
	 * strings.
	 * 
	 * @param id
	 *            The device's id.
	...
	/**
	 * Adds the device's value to the path. If the device's value type is
	 * binary, adds "true" if value != 0.0, else adds false. For decimal value
	 * types, adds the number as double.
	 * 
	 * @param device
	 *            The device.
	 */
	...
	/**
	 * Visits a concrete device and puts the value (optionally) and path of the
	 * device to the path buffer.
	 * 
	 * @param device
	 *            The device to visit.
	 */
